...
class GridWorldMDP(object):
    ...
    def __init__(self, noise=0.2, gamma=0.9):
        ...
        self.width = 5
        self.height = 5
        self.numstates = self.width*self.height + 1 # +1 for the extra absorbing state
    ...
    #         Pi: The action associated with each state (the policy) encoded as a 12x1 array
        ...
        while True:
            ...
            for i in range(0, self.numstates):
                ...
                # break
                for a in range(0, 4):
                    Rs[:, a] = Rs[:, a] + self.gamma*vPrev
                PR = Ps * Rs
                sum0 = sum(PR[:, 0])
                sum1 = sum(PR[:, 1])
                sum2 = sum(PR[:, 2])
                sum3 = sum(PR[:, 3])
                temp = [sum0, sum1, sum2, sum3]
                vNew[i] = max(temp)
                piNew[i] = temp.index(vNew[i])
            #print(vPrev)
            #print(vNew)
            # if n == 2:
            #     print(vNew)
            #     break
            if np.max(np.abs(vNew - vPrev)) > epsilon:
                vPrev = np.copy(vNew)
                piPrev = np.copy(piNew)
                n = n + 1
                # if n == 1:
                #     break
            else:
